...
ll <- sapply(lambdas, function(x){
  ll.poisson(x, sample)})

# store the lambdas and log likelihood results into a data frame
dflog <- data.frame(ll=ll, lambda=lambdas)
...
dflog%>%
  ...
  geom_vline(xintercept = lambdas[which.max(ll)],
             color = "red", size=2)
...
# Part c
# produce data which have Poisson distribution with the lambda=10
dat.poisson <- rpois(n=100, lambda=5) # as if lambda is unknown 
# reasonable range for the lambda
lambdas <- seq(0, 10, by=0.01)
...
function.2d = function(n){
  ...
  var.total.2d = vector()
  for (a in 1:1000){
  dat.d = rpois(n, lambda=10)
  mean.temp =mean(dat.d)
  var.temp =var(dat.d)
...
for (m in p){
  ...
  mean.temp =tmp[1]
  var.temp =tmp[2]
  mean.total.2e = append(mean.total.2e, mean.temp)
  var.total.2e = append(var.total.2e, var.temp)
}
